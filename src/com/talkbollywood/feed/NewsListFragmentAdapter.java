...
public class NewsListFragmentAdapter extends BaseAdapter implements ListAdapter, RssRequestListener, NewsItemEventListener{        
    ...
    public NewsListFragmentAdapter(Context context) {
    ...
    public void setNewsListListener(NewsListListener listener)
    {
    ...
    {               
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                boolean notifyFirstItemReady = false;
                if(items.size() < 1 && NewsListFragmentAdapter.this.listener!=null)
                {
                    notifyFirstItemReady = true;
                }
                items.clear();
                for(int i=0; i<values.size(); i++)
                {
                    items.add(new NewsItem(values.get(i), i, NewsListFragmentAdapter.this));                    
                }
                // Tell view/activity to fetch updated views
                NewsListFragmentAdapter.this.notifyDataSetChanged();    
                if(notifyFirstItemReady)
                {
                    NewsListFragmentAdapter.this.listener.onFirstItemsReady();
                }
            }
        });        
    }
